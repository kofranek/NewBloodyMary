...
DYMOLA_STATIC FMIStatus fmiSetFMUstate_(FMIComponent c, FMIFMUstate FMUstate)
{
	...
	/* set state vectors, derivatives, etc */
	{
		...
		case 1:
			...
			return util_error(source);
		case 2:
			util_logger(source, source->instanceName, FMIError, "", "%s failed, internal mismatch when copying variables", label);
			return util_error(source);
		default:
			util_logger(source, source->instanceName, FMIError, "", "%s failed, unhandled internal error code %d", label, ret);
			return util_error(source);
		}
												...
												FMIFMUstate* FMUstate)
{
	...
	if (tpl_load(tn, TPL_MEM, serializedState, size) != 0) {
		...
		for (j = 0; j < state->nSPar; j++) {
			comp->functions->freeMemory(state->sParameters[j]);
		}
		...
		// free fmu state expects memory to be allocated using allocateMemory
		// tpl_unpack uses something else, so we need to replace
		for (j = 0; j < state->nSPar; j++) {
			FMIChar *sParameter = state->sParameters[j];
			if (sParameter)
			{
				state->sParameters[j] = comp->functions->allocateMemory(MAX_STRING_SIZE + 1, sizeof(FMIChar));
...
static int copyVariables(Component* source, Component* target, const FMICallbackFunctions* funcs)
{
  ...
  if(target->did && dyn_allowMultipleInstances && source->did){
	  ...
	  if(nEobj){
		  ...
		  trgTmpTable = (struct ExternalTable_*) funcs->allocateMemory(nEobj,sz);
		  if(!trgTmpTable){
			ret = 1;
			goto clean1;
		  }
