...
DYMOLA_STATIC MarkObject DYNJPushMark(struct DYNFunctionData_*DYNStackData_) {
	MarkObject oldMark = *(DYNStackData_->mcurrentMark);
	DYNStackData_->mcurrentMark->mark = DYNStackData_->mcurrentMark->place;
	return oldMark;
}
DYMOLA_STATIC void DYNJRePushMark(MarkObject*oldMark, struct DYNFunctionData_*DYNStackData_) {
	MarkObject newMark = DYNJPushMark(DYNStackData_);
	if (oldMark) oldMark->place = newMark.place;
}
DYMOLA_STATIC void DYNJRelease(struct DYNFunctionData_*DYNStackData_) { DYNStackData_->mcurrentMark->place = DYNStackData_->mcurrentMark->mark; }
DYMOLA_STATIC void DYNJPopMark(MarkObject oldMark, struct DYNFunctionData_*DYNStackData_) { *(DYNStackData_->mcurrentMark) = oldMark; }
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC Real *DYNJRealTemp(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	Real *d = 0;
	d = DYNStackData_->mcurrentMark->place.Realbuffer;
	DYNStackData_->mcurrentMark->place.Realbuffer += r;
...
DYMOLA_STATIC Integer *DYNJIntegerTemp(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	Integer *i = 0;
	i = DYNStackData_->mcurrentMark->place.Integerbuffer;
	DYNStackData_->mcurrentMark->place.Integerbuffer += r;
...
DYMOLA_STATIC SizeType *DYNJSizeTemp(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	SizeType *s = 0;
	s = DYNStackData_->mcurrentMark->place.Sizebuffer;
	DYNStackData_->mcurrentMark->place.Sizebuffer += r;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealVaTemporarySize(SizeType ndims, struct DYNFunctionData_*DYNStackData_, va_list ap) {
	RealArray temp;
	SizeType i;
	temp.ndims = ndims;
	temp.dims = DYNJSizeTemp(ndims, DYNStackData_);
	for (i = 0; i<ndims; i++) temp.dims[i] = va_arg(ap, SizeType);
	return temp;
}
DYMOLA_STATIC RealArray DYNJRealVaTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, va_list ap) {
	RealArray temp = DYNJRealVaTemporarySize(ndims, DYNStackData_, ap);
	temp.data = DYNJRealTemp(RealNrElements(temp), DYNStackData_);
	return temp;
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealMatch(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	RealArray temp;
	SizeType i;
	temp.ndims = a.ndims;
	temp.dims = SizeTemp(a.ndims);
	for (i = 0; i<a.ndims; i++) temp.dims[i] = a.dims[i];
	temp.data = DYNJRealTemp(RealNrElements(a), DYNStackData_);
	return temp;
}
/* Construct array from sizes, using variable number of arguments */
DYMOLA_STATIC RealArray DYNJRealTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, ...) {
	RealArray temp;
	va_list ap;
	va_start(ap, DYNStackData_);
	temp = DYNJRealVaTemporary(ndims, DYNStackData_, ap);
	va_end(ap);
	return temp;
}
DYMOLA_STATIC RealArray DYNJRealTemporaryMatrix(SizeType r, SizeType c, struct DYNFunctionData_*DYNStackData_) {
	RealArray temp;
	temp.ndims = 2;
	temp.dims = DYNJSizeTemp(2, DYNStackData_);
	temp.dims[0] = r; temp.dims[1] = c;
	temp.data = DYNJRealTemp(r*c, DYNStackData_);
	return temp;
}
DYMOLA_STATIC RealArray DYNJRealTemporaryVector(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	RealArray temp;
	temp.ndims = 1;
	temp.dims = DYNJSizeTemp(1, DYNStackData_);
	temp.dims[0] = r;
	temp.data = DYNJRealTemp(r, DYNStackData_);
	return temp;
}
#endif
...
DYMOLA_STATIC Real RealElement(const RealArray a,...) {
  ...
  if (a.ndims == 1) {
	  SizeType j = va_arg(ap, SizeType);
	  Assert((j >= 1) && (j <= a.dims[0]), "Index out of bounds");
	  index = j - 1;
  } else {
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealArrayArray(SizeType narg, struct DYNFunctionData_*DYNStackData_, RealArray first, ...) {
	RealArray res;
	...
	res.data = DYNJRealTemp(prodsize*narg, DYNStackData_);
	va_start(ap, first);
	for (i = 0; i<narg; i++) {
		RealArray next;
...
DYMOLA_STATIC RealArray DYNJRealScalarArray(SizeType narg, struct DYNFunctionData_*DYNStackData_, ...) {
	RealArray res;
	SizeType i;
	va_list ap;
	res = DYNJRealTemporaryVector(narg, DYNStackData_);
	va_start(ap, DYNStackData_);
	for (i = 0; i<narg; i++) {
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealCat(SizeType along, struct DYNFunctionData_*DYNStackData_, SizeType nargs, RealArray first, ...) {
	RealArray res;
	va_list ap;
	SizeType i, j;
	res.ndims = first.ndims;
	res.dims = DYNJSizeTemp(res.ndims, DYNStackData_);
	...
	res.data = DYNJRealTemp(CommonNrElements(res.ndims, res.dims), DYNStackData_);
	{
		/* Pass 2: copy data */
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealPromote(const RealArray a, SizeType n, struct DYNFunctionData_*DYNStackData_) {
	RealArray res;
	SizeType i;
	Assert(n >= a.ndims, "Promote cannot decrease number of dimensions");
	res.ndims = n;
	res.data = a.data; /* No need to copy */
	res.dims = DYNJSizeTemp(n, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealPromoteScalar(const Real x, SizeType n, struct DYNFunctionData_*DYNStackData_) {
	RealArray res;
	Integer i;
	res.ndims = n;
	res.data = DYNJRealTemp(1, DYNStackData_);
	res.data[0] = x;
	res.dims = DYNJSizeTemp(n, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealGetSub(const RealArray a, struct DYNFunctionData_*DYNStackData_, ...) {
	RealArray temp;
	va_list ap;
	...
	temp.data = DYNJRealTemp(CommonNrElements(temp.ndims, temp.dims), DYNStackData_);
	/* Pass 3. Copy output */
	{
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealFill(const Real t, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	va_list ap;
	RealArray temp;
	va_start(ap, ndims);
	temp = RealFillAssign(DYNJRealVaTemporary(ndims, DYNStackData_, ap), t);
	va_end(ap);
	return temp;
}
DYMOLA_STATIC RealArray DYNJRealFillArray(const RealArray a, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	va_list ap;
	...
	temp.data = DYNJRealTemp(inputProd*fillProd, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealtranspose(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	RealArray res;
	Integer i, j, k, remsize;
	res.ndims = a.ndims;
	Assert(a.ndims >= 2, "transpose requires ndims>=2");
	res.dims = DYNJSizeTemp(a.ndims, DYNStackData_);
	res.dims[0] = a.dims[1];
	res.dims[1] = a.dims[0];
	remsize = 1;
	for (i = 2; i<a.ndims; i++) {
...
DYMOLA_STATIC RealArray DYNJRealouterproduct(const RealArray a, const RealArray b, struct DYNFunctionData_*DYNStackData_) {
	...
	Assert(b.ndims == 1, "Outerproduct require a vector as second argument");
	{
		...
		SizeType i, j;
		RealArray res = DYNJRealTemporaryMatrix(na, nb, DYNStackData_);
		for (i = 0; i<na; i++) {
			Real ai = a.data[i];
			for (j = 0; j<nb; j++)
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealAdd(const RealArray a, const RealArray b, struct DYNFunctionData_*DYNStackData_) {
	return RealAddAssign(DYNJRealMatch(a, DYNStackData_), a, b);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealSubtract(const RealArray a, const RealArray b, struct DYNFunctionData_*DYNStackData_) {
	return RealSubtractAssign(DYNJRealMatch(a, DYNStackData_), a, b);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealScale(const RealArray a, const Real t, struct DYNFunctionData_*DYNStackData_) {
	return RealScaleAssign(DYNJRealMatch(a, DYNStackData_), a, t);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealMinus(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	return RealMinusAssign(DYNJRealMatch(a, DYNStackData_), a);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealcross(const RealArray x, const RealArray y, struct DYNFunctionData_*DYNStackData_) {
	RealArray res = DYNJRealTemporaryVector(3, DYNStackData_);
...
DYMOLA_STATIC RealArray DYNJRealskew(const RealArray x, struct DYNFunctionData_*DYNStackData_) {
	RealArray res = DYNJRealTemporaryMatrix(3, 3, DYNStackData_);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealMultiplyMM(const RealArray a, const RealArray b, struct DYNFunctionData_*DYNStackData_) {
	return RealMultiplyMMAssign(DYNJRealTemporaryMatrix(a.dims[0], b.dims[1], DYNStackData_), a, b);
}
DYMOLA_STATIC RealArray DYNJRealMultiplyMV(const RealArray a, const RealArray b, struct DYNFunctionData_*DYNStackData_) {
	return RealMultiplyMVAssign(DYNJRealTemporaryVector(a.dims[0], DYNStackData_), a, b);
}
DYMOLA_STATIC RealArray DYNJRealMultiplyVM(const RealArray a, const RealArray b, struct DYNFunctionData_*DYNStackData_) {
	return RealMultiplyVMAssign(DYNJRealTemporaryVector(b.dims[1], DYNStackData_), a, b);
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealConvertInteger(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	RealArray res;
	SizeType i, n;
	res.ndims = a.ndims;
	res.dims = a.dims;
	n = CommonNrElements(res.ndims, res.dims);
	res.data = DYNJRealTemp(n, DYNStackData_);
	for (i = 0; i<n; i++) res.data[i] = a.data[i];
	return res;
}
DYMOLA_STATIC IntegerArray DYNJIntegerConvertReal(const RealArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	SizeType i, n;
	res.ndims = a.ndims;
	res.dims = a.dims;
	n = CommonNrElements(res.ndims, res.dims);
	res.data = DYNJIntegerTemp(n, DYNStackData_);
...
#endif
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealInverse(const RealArray A, struct DYNFunctionData_*DYNStackData_) {
	Assert(A.ndims == 2, "Can only invert matrices");
	Assert(A.dims[0] == A.dims[1], "Can only invert square matrices");
	{
		RealArray Ainv = DYNJRealMatch(A, DYNStackData_);
		MarkObject returnMark = DYNJPushMark(DYNStackData_);
		Real*RWork = DYNJRealTemp(A.dims[0] * A.dims[0] + 4 + 6 * A.dims[0], DYNStackData_);
		Integer*IWork = DYNJIntegerTemp(A.dims[0] * 2 + 1, DYNStackData_);
		...
		DYNJPopMark(returnMark, DYNStackData_);
		return Ainv;
	}
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealScaleDiv(const RealArray a, const Real t, struct DYNFunctionData_*DYNStackData_) {
	Assert(t != 0, "Division by zero in function RealScaleDiv");
	return RealScaleAssign(DYNJRealMatch(a, DYNStackData_), a, 1 / t);
}
#endif
...
/*  Internal: Use an existing va_list */
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerVaTemporarySize(SizeType ndims, struct DYNFunctionData_*DYNStackData_, va_list ap) {
	IntegerArray temp;
	SizeType i;
	temp.ndims = ndims;
	temp.dims = DYNJSizeTemp(ndims, DYNStackData_);
...
DYMOLA_STATIC IntegerArray DYNJIntegerVaTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_, va_list ap) {
	IntegerArray temp = DYNJIntegerVaTemporarySize(ndims, DYNStackData_, ap);
	temp.data = DYNJIntegerTemp(IntegerNrElements(temp), DYNStackData_);
	return temp;
}
/*  Construct a temporary of the same size as a */
DYMOLA_STATIC IntegerArray DYNJIntegerMatch(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray temp;
	SizeType i;
	temp.ndims = a.ndims;
	temp.dims = DYNJSizeTemp(a.ndims, DYNStackData_);
	for (i = 0; i<a.ndims; i++) temp.dims[i] = a.dims[i];
	temp.data = DYNJIntegerTemp(IntegerNrElements(a), DYNStackData_);
	return temp;
}
/* Construct array from sizes, using variable number of arguments */
DYMOLA_STATIC IntegerArray DYNJIntegerTemporary(SizeType ndims, struct DYNFunctionData_*DYNStackData_,...) {
	IntegerArray temp;
	va_list ap;
	va_start(ap, DYNStackData_);
	temp = DYNJIntegerVaTemporary(ndims, DYNStackData_, ap);
	va_end(ap);
	return temp;
}
DYMOLA_STATIC IntegerArray DYNJIntegerTemporaryMatrix(SizeType r, SizeType c, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray temp;
	temp.ndims = 2;
	temp.dims = DYNJSizeTemp(2, DYNStackData_);
	temp.dims[0] = r; temp.dims[1] = c;
	temp.data = DYNJIntegerTemp(r*c, DYNStackData_);
	return temp;
}
DYMOLA_STATIC IntegerArray DYNJIntegerTemporaryVector(SizeType r, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray temp;
	temp.ndims = 1;
	temp.dims = DYNJSizeTemp(1, DYNStackData_);
	temp.dims[0] = r;
	temp.data = DYNJIntegerTemp(r, DYNStackData_);
	return temp;
}
#endif
...
/* Construct Arrays from other arrays */
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerArrayArray(SizeType narg, struct DYNFunctionData_*DYNStackData_, IntegerArray first, ...) {
	IntegerArray res;
	SizeType i, j, prodsize;
	va_list ap;
	res.ndims = first.ndims + 1;
	...
	res.data = DYNJIntegerTemp(prodsize*narg, DYNStackData_);
	va_start(ap, first);
	for (i = 0; i<narg; i++) {
		IntegerArray next;
...
/* Construct arrays from scalars */
DYMOLA_STATIC IntegerArray DYNJIntegerScalarArray(SizeType narg, struct DYNFunctionData_*DYNStackData_, ...) {
	IntegerArray res;
	SizeType i;
	va_list ap;
	res = DYNJIntegerTemporaryVector(narg, DYNStackData_);
	va_start(ap, DYNStackData_);
	for (i = 0; i<narg; i++) {
...
/* Concatenate along dimensions 'along' (1..ndims) and given nargs arguments */
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerCat(SizeType along, struct DYNFunctionData_*DYNStackData_, SizeType nargs, IntegerArray first, ...) {
	IntegerArray res;
	va_list ap;
	SizeType i, j;
	res.ndims = first.ndims;
	...
	res.data = DYNJIntegerTemp(CommonNrElements(res.ndims, res.dims), DYNStackData_);
	{
		/* Pass 2: copy data */
		SizeType presize = 1;      /* Blocks */
		SizeType trailingsize = 1; /* Elements in each block */
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerPromoteScalar(const Integer x, SizeType n, struct DYNFunctionData_*DYNStackData_) {
	IntegerArray res;
	Integer i;
	res.ndims = n;
	res.data = DYNJIntegerTemp(1, DYNStackData_);
	res.data[0] = x;
	res.dims = DYNJSizeTemp(n, DYNStackData_);
	for (i = 0; i<n; i++) res.dims[i] = 1;
	return res;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerGetSub(const IntegerArray a, struct DYNFunctionData_*DYNStackData_, ...) {
	IntegerArray temp;
	va_list ap;
	SizeType ndims = a.ndims, trailingsize = 1, nargs = 0;
	/* Pass 1. Determine nr. dimensions of output */
	...
	temp.data = DYNJIntegerTemp(CommonNrElements(temp.ndims, temp.dims), DYNStackData_);
	/* Pass 3. Copy output */
	{
		struct TaggedIndexStack*apt = 0;
		struct TaggedIndexStack VA_handler[100];
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerCopy(const IntegerArray a, struct DYNFunctionData_*DYNStackData_) {
	int i, imax;
	IntegerArray temp;
	temp.ndims = a.ndims;
	temp.dims = DYNJSizeTemp(a.ndims, DYNStackData_);
	...
	temp.data = DYNJIntegerTemp(imax, DYNStackData_);
	for (i = 0; i<imax; ++i) temp.data[i] = a.data[i];
	return temp;
}
DYMOLA_STATIC void DYNJIntegerPutSub(const IntegerArray ain, struct DYNFunctionData_*DYNStackData_, IntegerArray out, ...) {
	IntegerArray a;
	va_list ap;
	SizeType out_dim = 0, a_dim = 0, trailingsize = 1, nargs = 0;
	a = DYNJIntegerCopy(ain, DYNStackData_);
	/* Pass 1. Determine nr. dimensions of output */
	{
		Integer over = 0;
		va_start(ap, out);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerFill(const Integer t, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	va_list ap;
	IntegerArray temp;
	va_start(ap, ndims);
	temp = IntegerFillAssign(DYNJIntegerVaTemporary(ndims, DYNStackData_, ap), t);
	va_end(ap);
	return temp;
}
DYMOLA_STATIC IntegerArray DYNJIntegerFillArray(const IntegerArray a, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	va_list ap;
	int i, j, inputProd, fillProd;
	IntegerArray temp;
	temp.ndims = a.ndims + ndims;
	temp.dims = DYNJSizeTemp(temp.ndims, DYNStackData_);
	...
	temp.data = DYNJIntegerTemp(inputProd*fillProd, DYNStackData_);
	for (i = 0; i<inputProd; ++i) {
		for (j = 0; j<fillProd; j++) {
			temp.data[i + j*inputProd] = a.data[i];
		}
...
/* For from:stride:to */
#if defined(DYNCallStackData)
DYMOLA_STATIC IntegerArray DYNJIntegerRange(const Integer from, const Integer to, const Integer stride, struct DYNFunctionData_*DYNStackData_) {
	Integer num;
	Assert(stride != 0, "range requires non-zero stride");
	num = (Integer)floor((to - from)*1.0 / stride) + 1;
	if (num<0) num = 0;
	{
		IntegerArray res = DYNJIntegerTemporaryVector(num, DYNStackData_);
		Integer i;
...
#if defined(DYNCallStackData)
DYMOLA_STATIC void DYNJPopMarkMarks(struct DYNFunctionData_*DYNStackData_) {
	*(DYNStackData_->mcurrentMark) = *(DYNStackData_->mstartMark);
...
#if defined(DYNCallStackData)
DYMOLA_STATIC void DYNJSetStringMark(char*s, struct DYNFunctionData_*DYNStackData_) {
	if (s) stringmark = s;
}
#endif
...
#if defined(DYNCallStackData)
DYMOLA_STATIC RealArray DYNJRealTemporaryDense(Real*d, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	RealArray temp;
	va_list ap;
	va_start(ap, ndims);
	temp = DYNJRealVaTemporarySize(ndims, DYNStackData_, ap);
	va_end(ap);
	temp.data = d;
	return temp;
}
DYMOLA_STATIC IntegerArray DYNJIntegerTemporaryDense(Real*d, struct DYNFunctionData_*DYNStackData_, SizeType ndims, ...) {
	IntegerArray temp;
	int i, nrElem;
	va_list ap;
	va_start(ap, ndims);
...
DYMOLA_STATIC char* ModelicaDuplicateString(const char*str) {
	char*s = 0;
	if (str == 0) ModelicaFormatError("ModelicaDuplicateString with null-pointer");
	else {
		s = StringAllocate(strlen(str) + 1);
		strcpy(s, str);
	}
	return s;
}
DYMOLA_STATIC char* ModelicaDuplicateStringWithErrorReturn(const char*str) {
	EnsureMarkInitialized();
	if (str && (stringmark + strlen(str) + 1<Endsimplestring))
		return ModelicaDuplicateString(str);
	else return 0;
}
