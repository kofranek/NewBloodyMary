...
struct DymosimCallBacks {
	int(*stepEventCallBack)(struct DymosimSimulator*);
	void(*updateCallBack)(struct DymosimSimulator*,double* time,
		double* xvec,
		double* dxvec,
		double* uvec,
		double* dpvec,
...
struct SimulatorFunctions {
	...
	void(*setStoreIncrement)(struct DymosimSimulator* sim, double hIncr);
	int (*mainModelCallbacks)(int, char *[], const struct DymosimModel*, const struct DymosimCallBacks*);
};
#include "dsblock.h"
...
struct DymosimSimulator { 
	...
	struct  {
		...
		double dsfinal_snapshot_interval;
		double dsfinal_last_snapshot;
	} dsRes;
	...
	int enforceDyRefresh;
	struct DymosimCallBacks callBacks;
};
