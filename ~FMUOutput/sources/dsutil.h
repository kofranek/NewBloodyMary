...
#define DYNEXTRABEGIN3 struct DYNFunctionData_ dynFuncData, *DYNStackData_;EnsureMarkInitialized();dynFuncData.mstartMark = &startMark;dynFuncData.mcurrentMark = &currentMark;DYNStackData_ = &dynFuncData;
#else 
#define DYNEXTRABEGIN3 
#endif
#if defined(LIBDS_DLL) && defined(DYN_MULTINSTANCE)
#define DYNEXTRABEGIN1 struct DymosimSimulator*saved_threadData;saved_threadData=dymosimGetThreadData();
#define DYNEXTRABEGIN2 dymosimSetThreadData(saved_threadData);
#else
#define DYNEXTRABEGIN1
#define DYNEXTRABEGIN2
#endif
static void DYNMergeBasic(struct BasicDDymosimStruct*DYNBO, const struct BasicDDymosimStruct*DYNB) {
	if (DYNBO && DYNB) {
		if (DYNB->mNextTimeEvent<1e30 && (DYNBO->mNextTimeEvent >= 1e30 || DYNB->mStartNextTimeEvent < DYNBO->mStartNextTimeEvent)) {
			DYNBO->mNextTimeEvent = DYNB->mNextTimeEvent;
			DYNBO->mStartNextTimeEvent = DYNB->mStartNextTimeEvent;
		}
		if (DYNB->mOrigTimeError > DYNBO->mOrigTimeError)
			DYNBO->mOrigTimeError = DYNB->mOrigTimeError;
		if (DYNB->mCurrentStepSizeRatio2 != 1)
			DYNBO->mCurrentStepSizeRatio2 = DYNB->mCurrentStepSizeRatio2;
	}
}
#define beginJmpParallelSections(n) {struct BasicDDymosimStruct DYNB[n];struct BasicDDymosimStruct*DYNBO=0;struct MyJmpBuf saved_jmp_buf_env;int DYNLocalError=0;int DYNParallelGoto=0;DYNEXTRABEGIN1 saved_jmp_buf_env=DYNGlobal_jmp_buf_env;{int i;DYNBO=(struct BasicDDymosimStruct*)(duser_);for(i=0;i<n;i++) DYNB[i]=*DYNBO;}
#define beginJmpParallelSection(i) DYNEXTRABEGIN2 DYNPropagateDidToThread(did_);if (setjmp(DYNGlobal_jmp_buf_env.TheBuffer)==0) {double*duser_=(double*)&DYNB[i];double DYNT=*time;double*time=&DYNT;DYNEXTRABEGIN3
#define endJmpParallelSection(i) } else DYNLocalError=1;
#define endJmpParallelSections(n) DYNGlobal_jmp_buf_env=saved_jmp_buf_env;if (DYNLocalError) {GlobalError_=DYNLocalError;longjmp(DYNGlobal_jmp_buf_env.TheBuffer, 1);} \
	if (DYNParallelGoto == 1) { goto finish; } else if (DYNParallelGoto == 2) { goto leave; }\
    {int i;for(i=0;i<n;i++) {DYNMergeBasic(DYNBO,DYNB+i);}}}
...
#if defined(_MCCPPC) /* use old definition for dSPACE DS1005 Microtec compiler */ 
...
#else
#define PushContext(a) int oldFunctionExternal=FunctionExternalContext_;struct FunctionContext_ alpha_={a,FunctionContextGlobal_};FunctionContextGlobal_=&alpha_;
#endif
...
DYMOLA_STATIC void Dymola_Simulation_SetOutputFrequency(double d) {
#if defined(DYMOSIM)
	blosst_(&d);
#endif
}
...
#define HandleEvent(cross,expr,index) \
	...
	handleevent3(expr, &QM_[index], &Qp_[index-1], &Qn_[index-1], QRel_[index-1], \
		Init_, FirstEvent?-PrintEvent:PrintEvent, EPS_, *time, &AnyEvent_, Qenable_[index], LargeEPS_, Iter>SlowIter_); \
    } \
    if (RootFinder_) { \
...
#define HandleEvent(cross,expr,index) \
    QRel_[index-1] = cross; \
    if (DYNEvent) { \
	if (FirstEvent) PrintEvent = -PrintEvent; \
...
#define HandleEvent2A(cross,xexpr,rel,yexpr,index,ltz,invres) \
    ((QRel_[index-1] = cross, \
    ((DYNEvent) ? \
    (handleevent4(xexpr " " rel " " yexpr,"(" xexpr ")-(" yexpr ")",&QL_[index], &Qp_[index-1],&Qn_[index-1],EPS_*Qscaled_[index-1],QRel_[index-1],Init_,FirstEvent?-PrintEvent:PrintEvent,&AnyEvent_,ltz,invres,Qenable_[index],LargeEPS_*Qscaled_[index-1],Iter>SlowIter_,index),0):0), \
      (DSE_STRUCT QZ_)[2*(index)-2] = QRel_[index-1] - Qp_[index-1], \
...
#define roundEventMinor(cross,func,expr,index,posi,negi,reli) \
	( DYNX(QRel_,index-1) = cross,\
		...
		posi,negi,reli,Init_,FirstEvent?-PrintEvent:PrintEvent,&AnyEvent_,DYNX(Qenable_,index),LargeEPS_,Iter>SlowIter_,index),0):0),\
...
#define roundEvent(cross,func,expr,index,posi,negi,reli) \
	( DYNX(QRel_,index-1) = cross,\
		...
		posi,negi,reli,Init_,FirstEvent?-PrintEvent:PrintEvent,&AnyEvent_,&AnyDEvent_,DYNX(Qenable_,index),LargeEPS_,Iter>SlowIter_,index),0):0),\
		DYNX((DSE_STRUCT QZ_),2*(index)-2) = -(DYNX(QRel_,index-1)-DYNX(Qp_,index-1)+DYNX(Qn_,index-1)+((posi)?(reli?(DYNX(Qp_,index-1)<=0):1):0)),\
		DYNX((DSE_STRUCT QZ_),2*(index)-1) = -(DYNX(QRel_,index-1)-DYNX(Qp_,index-1)-DYNX(Qn_,index-1)-((negi)?(reli?(DYNX(Qp_,index-1)>=0):1):0)),\
		DYNX(Qenable_,index)=true,DYNX(Qp_,index-1))
...
#define DYN_REMEMBER_DECL ,const int eq_remember_accepted_variable_,const int eq_remember_enable_linear,const int eq_remember_1_time_,const double eq_remember_2_time_,const double eq_remember_time_factor
#define DYN_REMEMBER_CALL ,eq_remember_accepted_variable_,eq_remember_enable_linear,eq_remember_1_time_,eq_remember_2_time_,eq_remember_time_factor


#define BreakSectionIVDef(nr) \
	DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr,RealArray x__,Real*x____internal,const int veryFirstTime,const int firstTime,const double lambda_,const Real homlocal,const int DAEsolverMode DYN_REMEMBER_DECL);

#define BreakSectionIVCallNewName(nr,name) \
   { switch(equationsJ_##nr(idemand_, icall_, \
      time, X_, XD_, U_, \
      DP_, IP_, LP_, \
      F_, Y_, W_, QZ_, \
      duser_, iuser_, cuser_,did_ DYNOPTCallCSD, \
      QiErr,name,name##__internal,veryFirstTime, firstTime, lambda_,homlocal,DAEsolverMode DYN_REMEMBER_CALL)) { case 1:goto finish;case 2:goto leave;default:;} \
	}


#define BreakSectionIVEnd() \
	   return 0;finish: return 1;leave:return 2;}

#define BreakSectionIVStart(nr) \
	   DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      double *time, double* X_, double* XD_, double* U_, \
      double* DP_, int IP_[], Dymola_bool LP_[], \
      double* F_, double* Y_, double* W_,  double QZ_[], \
...
#define BreakSectionIVCallNew(nr) BreakSectionIVCallNewName(nr, x__)
...
#define BreakSectionCallNew(nr) \
   { switch(equationsJ_##nr((DSE_STRUCT idemand_), (DSE_STRUCT icall_), \
      ...
      (DSE_STRUCT duser_), (DSE_STRUCT iuser_), (DSE_STRUCT cuser_), (DSE_STRUCT did_) DYNOPTCallCSD, \
...
#define EndPreBlock if (!final_ && (DymolaOneIteration_==3) && ((!FirstEvent && PrintEvent&(1 << 1)) || (FirstEvent && PrintEvent&(1 << 2)))) DymosimMessage("Iterating to find consistent restart conditions.");}
...
DYMOLA_STATIC void registerTimeEventNew(const double atTime, struct BasicDDymosimStruct*basicD);
...
DYMOLA_STATIC int DYNTimeEvent2(double y,int less, struct BasicDDymosimStruct*duser, double time, double origTimeError, Dymola_bool*AnyEvent, Dymola_bool*AnyDEvent,int pr,int DymolaOneIteration, Dymola_bool*qt) {
	if (!AnyEvent || !qt) return 0;
	if (time < y - (4e-14)*(fabs(time) + origTimeError)) {
		registerTimeEventNew(y, duser);
		if (*qt) {
...
#define DYNTimeEvent(y,nr,less) \
  (((Init_||DYNEvent) ? DYNTimeEvent2(y,less,(struct BasicDDymosimStruct*)(DSE_STRUCT duser_),DYNTime, ((struct BasicDDymosimStruct*)(DSE_STRUCT duser_))->mOrigTimeError, &AnyEvent_, &AnyDEvent_, (!FirstEvent && PrintEvent&(1 << 1)) || (FirstEvent && PrintEvent&(1 << 2)), DymolaOneIteration_, &DYNX(QTimed_,nr)):0),DYNX(QTimed_,nr))
/* Deactivate the state event */

#define DYNTimeEventMinor(y,nr,less) \
  (((Init_||DYNEvent) ? DYNTimeEvent2(y,less,(struct BasicDDymosimStruct*)(DSE_STRUCT duser_),DYNTime, ((struct BasicDDymosimStruct*)(DSE_STRUCT duser_))->mOrigTimeError, &AnyEvent_, 0          , (!FirstEvent && PrintEvent&(1 << 1)) || (FirstEvent && PrintEvent&(1 << 2)), DymolaOneIteration_, &DYNX(QTimed_,nr)):0),DYNX(QTimed_,nr))
/* Deactivate the state event */


#define floorTimeEvent(d, y, str, nr) \
	((Init_||DYNEvent)?DYNTimeFloorEvent(d, y, did_, DYNTime, &AnyEvent_, &AnyDEvent_, nr, (!FirstEvent && PrintEvent&(1 << 1)) || (FirstEvent && PrintEvent&(1 << 2)), DymolaOneIteration_, str):((did_->NextSampleTime_vec[nr]>=0)?(did_->NextSampleTime_vec[nr]-1):did_->NextSampleTime_vec[nr]))
#define floorTimeEventMinor(d, y, str, nr) \
	((Init_||DYNEvent)?DYNTimeFloorEvent(d, y, did_, DYNTime, &AnyEvent_, 0          , nr, (!FirstEvent && PrintEvent&(1 << 1)) || (FirstEvent && PrintEvent&(1 << 2)), DymolaOneIteration_, str):((did_->NextSampleTime_vec[nr]>=0)?(did_->NextSampleTime_vec[nr]-1):did_->NextSampleTime_vec[nr]))
...
#define whenModelicaS3(iv,prev1,curr1,iVar,hVar,cond1)\
	ResetIfNewSimulation(prev1,-1e30,iv);\
	whenModelicaS(((cond1) || Init_ || (DYNEvent && prev1==DYNTime))) \
	iVar=!((InitTime==DYNTime) ? (!Init_ && Iter==2): (prev1!=DYNTime)); \
    if (prev1==DYNTime || prev1<-1e29) curr1=0;else curr1=DYNTime-prev1;prev1=DYNTime;hVar=curr1;
...
#define RosenBrockSolverMostb(n, cs) \
		double oldT = DYNTime; \
		rosenMark = PushMark(); \
		SetNeedFactor(A); \
		RealFillAssign(A, 0.0); \
		RealFillAssign(E, 0.0); \
		for (i = 1; i <= (cs>0) ? Nstages + 1 : 1; ++i) { \
			NewStep = (i == 1); \
			FinalStage =(cs>0) ? (i == Nstages + 1) : 1; \
			DYNTime = oldT + cs * (c2[i - 1] -1)

#define RosenBrockSolverMost1(n, cs) \
		RosenBrockSolverMosta(n);\
		double u[n * 1 + 1] = {0}; \
		RosenBrockMethod1a; \
		RosenBrockSolverMostb(n, cs)

#define RosenBrockSolverMost2(n, cs) \
		RosenBrockSolverMosta(n);\
		double u[n * 2 + 1] = {0}; \
		RosenBrockMethod2a; \
		RosenBrockSolverMostb(n, cs)

#define RosenBrockSolverMost3(n, cs) \
		RosenBrockSolverMosta(n);\
		double u[n * 2 + 1] = {0}; \
		RosenBrockMethod3a; \
		RosenBrockSolverMostb(n, cs)

#define RosenBrockSolverMost4(n, cs) \
		RosenBrockSolverMosta(n);\
		double u[n * 4 + 1] = {0}; \
		RosenBrockMethod4a; \
		RosenBrockSolverMostb(n, cs)

#define RosenBrockSolver1(n, r, cs) \
	if (cs > 0 || r) { \
		RosenBrockSolverMost1(n, cs)

#define RosenBrockSolver2(n, r, cs) \
	if (cs > 0 || r) { \
		RosenBrockSolverMost2(n, cs)

#define RosenBrockSolver3(n, r, cs) \
	if (cs > 0 || r) { \
		RosenBrockSolverMost3(n, cs)

#define RosenBrockSolver4(n, r, cs) \
	if (cs > 0 || r) { \
		RosenBrockSolverMost4(n, cs)

#define RosenBrockSolverNA1(n, r, cs) \
	if (cs > 0 || r) { \
		DeclareRealVector(fd, n); \
		RosenBrockMethod1b; \
		RosenBrockSolverMost1(n, cs)

#define RosenBrockSolverNA2(n, r, cs) \
	if (cs > 0 || r) { \
		DeclareRealVector(fd, n); \
		RosenBrockMethod2b; \
		RosenBrockSolverMost2(n, cs)

#define RosenBrockSolverNA3(n, r, cs) \
	if (cs > 0 || r) { \
		DeclareRealVector(fd, n); \
		RosenBrockMethod3b; \
		RosenBrockSolverMost3(n, cs)

#define RosenBrockSolverNA4(n, r, cs) \
	if (cs > 0 || r) { \
		DeclareRealVector(fd, n); \
		RosenBrockMethod4b; \
		RosenBrockSolverMost4(n, cs)
...
#ifdef RosenEuler
#define RosenBrockSolverNA(n,r) RosenBrockSolverNA1(n,r,cs)
#define RosenBrockSolver(n,r) RosenBrockSolver1(n,r,cs)
#else
#define RosenBrockSolverNA(n,r) RosenBrockSolverNA3(n,r,cs)
#define RosenBrockSolver(n,r) RosenBrockSolver3(n,r,cs)
#endif

#define EndRosenBrockSolver(n, cs) \
			if (!(cs>0)) ;\
			else if (NewStep) { \
				double dx = 1 / (gamma * cs); \
				for (j = 0; j < n * n; ++j) { \
...
#define NARosenBrockSolver(n, cs) \
	{ \
		double gf = cs * g2[(i - 1)]; \
		for (j = 0; j < n; ++j) { \
...
LIBDS_API  int dymli1_(int* sysnr, const int *fact, double* a, int* lda, int* n, 
                   ...
                   double* dwork, int* iwork, int* ierr, const char* tag);
...
LIBDS_API  int dymli5_(int* sysnr, int *fact, double *a, int* lda, int *n,
		   ...
		   double* dwork, int* iwork, int *factor, const char*const*varnames,int* ierr,int*fEvent,double*intTol, const char* tag);


LIBDS_API  int dymlin_(int* infrev, int* n, double* sol, double* res, double *jac,
                   int* ljac, double* dwork, int *iwork, double *Time,
                   int* event, int *printpriority, int *sysnr, int *ierr, const char* tag);
...
#define DymSol(A, Max, n, b, Dwork, Iwork, ierr_) \
    if ( n > MaxMat ) { \
	  ...
	  {int i1_,i2_,i3;i1_ = MaxMat;i2_ = n;i3_ = 0; \
	   dymli1_(&i3_, &i3_, qa_, &i1_, &i2_, qb_, &DYNTime, &Event, &PrintEvent, dwork_, iwork_, ierr_, 0);}
...
#define DymLin(QInfRev,QiOpt,QNnl,QSol,QRes,QJac,Qtol,Qinfo,QD,QI, \
	...
	dymlin_(&QInfRev,&QNnl,QSol_,QRes_,QJac_,&i1_,dwork_,iwork_,&DYNTime,&i3_,&PrintEvent,&i2_,ierr_,0);}
