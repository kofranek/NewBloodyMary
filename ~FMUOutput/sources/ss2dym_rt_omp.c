...
#include <mpadvise.h>
#include <omp.h>
#if defined(DYM2CCUR)  && defined(OMP_DYMOLA) && !defined(NUM_THREADS)
/* Definining number of threads sets an upper bound for number of threads. Having it equal to number of physical cores may help. */
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	*/
    ...
    ssSetNumContStates(    S, nx);     /* number of continuous states */
	{
		...
		EventInUpdate=!DirectFeedThrough || (basicI.mInlineIntegration && DYNEventInUpdate_);
	}
#if GenerateResultInNonDymosim
	if (1)
#else
